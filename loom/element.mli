(** What a component returns: a description of what should exist right now.
    ...
    That separation is what the design rests on: a game rebuilds the entire
    description every frame, with no bookkeeping, and the reconciler works out
    ...
    The first consequence is a rule, React's rule for React's reason:
    ...
    closure every frame. It is therefore never the same component as last frame,
    so it is torn down and rebuilt every frame along with everything under it.
    The symptom is that state appears not to work. {!declare} exists partly to
    make the right thing the easy thing: the closure it captures is made once,
    when the module is initialised.

    The second consequence runs the other way — not a component that is
    accidentally two, but two that are accidentally one.
    ...
    Give a [key] to anything that can be rearranged. Among a parent's children,
    a keyed element is matched to last frame's by its key alone and an unkeyed
    one by its position. A list of enemies that sorts itself by distance keeps
    each enemy's state only if the enemies are keyed. See {!Path} for the same
    rule stated from the identity end.
    ...
    A {!Fragment} can be keyed like anything else. A helper returning several
    primitives at once needs that: there is no one primitive to put the key on,
    and without it a list of such things cannot be rearranged without losing
    what is inside them. *)
...
(** Two of one parent's children were given the same key. [at] is the parent's
    ...
    Raised while reconciling, so it is a frame the host never assembled: the
    tree from the frame before is still standing and nothing has been committed.
    See {!Reconcile}. *)
...
(** A component's own function raised [Invalid_argument] while describing
    ...
    This is a translation, not a new failure: the runtime turns the exception
    ...
    moment the call is made. Without it the message is a complaint about a world
    with nothing to attach it to, useless to a reader looking for the line to
    change.
    ...
    body around it is, so a refusal out of one reads the same as a refusal out
...
type 'prim t =
  ...
  | Component : {
      ...
      -> 'prim t
      (** a component and the props to call it with. ['props] is existential so
          that a parent may hold children of many different prop types in one
          list; only the component itself ever sees its own. *)
...
(** The key this element was given, if it was given one. {!Empty} never has one,
    having nothing to keep. Neither does {!Provide}: a context binding is not a
    thing a game rearranges, and nothing has asked to key one. *)
