(** Everything the runtime needs to know about the thing it is driving, and
    nothing more.
    ...
    Two things are deliberately absent, both for the same reason: a host has no
    [create], [update] or [destroy], and it is handed the finished forest rather
    than a stream of mutations. React's host config is written the other way
    round because the DOM is a mutable tree that must be poked node by node. A
    raycaster's world is not: it is an immutable value assembled whole, and the
    primitives that go into it are cheap records. Handing over the finished
    description and asking for a scene is simpler and a better fit.

    It also keeps the interface small enough that a mock host is a dozen lines,
    ...
    around. For this engine's host it measures at a seventh of one percent of
    what drawing the same frame costs, so nothing here designs around it. The
    paths in the forest are stable across frames all the same, so an assembler
    that wants to cache what a subtree built can, without anything above it
    changing. *)
...
(** One primitive, in place, with what it contains.
    ...
    representation here: a component's whole job is to explain itself in terms
    of things a host understands, and by this point it has.
    ...
    parenthood; that logic should exist in one place. *)
...
module type HOST = sig
  ...
  *)
  ...
  (** Turn a frame's finished description into something drawable.
      ...
      wrote them. Write it as a pure function of what it is given: the
      reconciler will call it with an equal forest and expect an equal scene. A
      cache keyed on {!Path.t} is the supported way to make it fast.

      Errors in what a game described — a room that does not close, a doorway
      linked twice — are the host's to detect, because the runtime cannot tell a
