...
Files written:
    ...
    assets/font.png          96x60  RGBA      96 cells of 6x10: printable ASCII
...
# --------------------------------------------------------------------------
# assets/font.png — the typeface, as a fixed grid.
#
# 96 cells of 6 x 10, sixteen across, holding printable ASCII from 32 up. The
# 96th is a box, which Font uses for anything outside that range.
#
# Each glyph is one line: rows top to bottom, `#` on and `.` off, five wide.
# Rows a glyph does not reach are blank, which is why the lowercase letters
# start two rows in — that is their x-height. Rows 7 and 8 are below the
# baseline and only the descenders use them; row 9 is the leading, and is always
# clear.
#
# Written out in full rather than derived, because a typeface is the one kind of
# art where every pixel is a decision someone has to have made.
# --------------------------------------------------------------------------

CELL_W, CELL_H, COLUMNS, FIRST = 6, 10, 16, 32
...
def font_cells():
    """Cell index -> list of row strings, for all 96."""
    cells = {}
    for i in range(95):
        cells[i] = GLYPHS[chr(FIRST + i)].split()
    cells[95] = TOFU.split()
    return cells
...
def font(x, y):
    col, gx = divmod(x, CELL_W)
    row, gy = divmod(y, CELL_H)
    rows = _CELLS.get((row * COLUMNS) + col, [])
    lit = gy < len(rows) and gx < len(rows[gy]) and rows[gy][gx] == "#"
    # White, so that Paint's tint — which multiplies — reproduces whatever
    # colour was asked for exactly. The alpha is the whole of the shape.
    return (255, 255, 255, 255 if lit else 0)
...
if __name__ == "__main__":
    ...
    write_png("assets/font.png", COLUMNS * CELL_W, 6 * CELL_H, font)
