(** What the player is looking at: the crosshair's ray, traced through doorways
    ...
    drawing a column of a neighbouring room in the opening, transformed into its
    frame. This module asks the same question of the middle column and answers
    ...
    room — and never with the thing itself. An index survives
    {!World.replace_room}, which a game animating what the player is looking at
    calls every frame; a copy of the wall would be a picture of how it used to
    look. What those indices {e mean} is the game's own business: the engine has
    no notion of a sign, only of the sprite that happens to be one.
    ...
    The same things that stop the eye: what can be picked is what can be seen.

    - A {b wall}, where it is solid. The ray's height has to fall between the
      wall's foot on the sloped floor and its top; otherwise it passes over or
      under. The top is the wall's own height or the ceiling standing over that
      point, whichever is lower: {!Renderer} caps a wall at the ceiling and
      paints the ceiling above it, so a wall that runs on past one is a wall
      nobody can see up there. A room open to the sky caps nothing. Solid is
      asked of the {e texel} the crosshair is on and not of the wall as a whole,
      by {!Material.opaque_at}, because that is the question the renderer asks
      of each pixel: a texel it writes outright hides what is behind, and a
      texel it merely blends — the gap in a grille, a pane of glass — is one the
      eye goes through. The bars of a grille therefore stop the ray and the
      holes between them do not, which is what the picture shows. One ray a
      frame can afford the per-texel question; the renderer, at one ray a
      column, routes a whole wall by {!Material.opaque} first and reaches the
      same answer.
    - A {b decal} on either sort of wall, where its image is not transparent
      there. This makes the see-through case an exception rather than a hole:
      the renderer draws a wall's decals whether or not the wall itself was
      drawn, so a mark painted on glass is visible, and what is visible is what
      can be picked. The wall it is on is what gets named — a decal is something
      on a wall, never a thing of its own.
    - A {b sprite}, where its image is not transparent there. Sprites are cut
      out against {!Image.clear} and mostly empty, so this too is asked of the
      texel and not of the bounding box: a crosshair between a figure's arm and
      its side is looking at what is behind it. A sprite nearer than
      ...
      two cannot drift apart; a sprite the frame does not show is not something
      the crosshair should claim to be looking at.
    ...
    - Running out of {b doorways to look through}. By default the renderer runs
      out of them at the same point, and the opening the ray stops at is the one
      the frame filled with haze.

    A doorway is bounded above the same way a wall is, and for the same reason.
    A lintel reaches its own [top] and no further; the ceiling may cut it
    shorter, or the opening itself shorter still. Past whichever of the three
    comes first, the answer is the bare case again whatever hangs there: nothing
    to pick, and no way through. The renderer has drawn this room's own ceiling
    across those rows, and neither the strip nor the neighbour reaches them.

    {1 Depth and distance}

    [through] is how many doorways the ray may pass. It defaults to
    {!Config.max_portal_depth} — the renderer's own budget, read from where the
    renderer reads it. That is the depth at which the two agree: at any less,
    the crosshair reports the doorway while the picture shows what is beyond it.
    There is no more to ask for: past that budget the frame has stopped drawing
    rooms and filled the opening with haze. It is the same arrangement
    ...
    the player and no further. But "how far away may a thing be worked from" is
    usually a question about {!t.distance} or {!t.crossed} rather than about the
    cast. Those describe what was found, and refusing it is the game's to do; a
    shortened ray refuses by not looking and cannot tell what it declined to
    see.
    ...
    behind the player is. It is the same rule and the same number the renderer
    clips its portals with, which is how "what can be picked is what can be
    seen" survives a room that folds. *)

(** What the ray met. Concrete because matching on it is the intended use — see
...
type kind =
  | Wall of {
      ...
      (** Which wall of the room, how far along it from its [a] endpoint, how
          ...
          Decals are counted from the head of {!Room.type-wall}'s list; where
          two overlap, the topmost is reported, since it is drawn last and is
          the one visible.

          The first four are what {!Room.add_decal} wants: [index] says which
          wall, [along] and [z] are that wall's own coordinates — the ones a
...
type t = {
  ...
  pose : Player.t;
      (** The player, in {!t.room}'s own frame: the same eye, expressed where
          the thing it is looking at lives. Anything wanting to {e draw} the
          target needs this, since the room it is in has its own coordinates;
          {!Viewport.sprite_box} takes this. Where the ray crossed no doorway it
          is the player unchanged. *)
  kind : kind;
}
...
(** [look world player] is what [player] has the crosshair on, or [None] if the
    ray met nothing: aimed at the sky, below every wall it passed, or out of
    ...
    size the window has been dragged to. *)
