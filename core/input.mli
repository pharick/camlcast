(** Raw input state: what the hardware reports, and nothing about what it means.
    Nothing here knows about positions or walls, and since {!Binding} nothing
    here knows which key walks forward either. This module reads the keyboard
    and the mouse, counts the edges and the holds, and hands the result on for
...
(** What the player asks for over one frame, as finished per-frame deltas, so
    {!Camlcast_core.Engine} can apply it straight to the
...
type control =
  ...
  | Button of button
      (** Something the player can hold down. Keys are places on the keyboard
          ...
          The engine has no opinion about what any control means. "Interact",
          "chalk", "journal" are a game's words for its own table from controls
          to actions. The engine owns the mechanism underneath: when a control
          went down, when it came up, and how long it was held. Even walking is
          a game's table now — {!Binding} holds it, and the engine's only part
          in that is a default. *)
...
(** What an analog source's number means: the one thing {!Binding} needs in
    order to scale it. The mouse reports where it has been; a stick reports how
    hard it is being pushed. The two are not the same kind of number, however
    alike they look. *)
...
(** What the player is holding, what they have just done, and what the analog
    ...
    Abstract, and of everything here the abstraction most worth having. Inside
    are three arrays indexed by control; neither how they are indexed nor that
    they exist is a caller's business. The sharper reason: the frame handed to a
    game {e is} the frame the next one is built from. {!advance} makes the new
    frame's [was_down] the previous frame's [down] — the same array, not a copy
    — and {!untouched} is one value the whole process starts every run from.
    Were those arrays reachable, a single write from a game's [update] would
    rewrite the edges of the frame after it, and a write that reached
    [untouched] would outlive the run that made it. That is not a rule a caller
    could be asked to keep, so the type does not ask.

    The cost is one function, {!with_pointer}, for the one update from outside
    this module that a frame of input legitimately has. *)

val untouched : actions
(** Nothing held, nothing pressed, nothing moved, the cursor at the origin.
    Sharing this one value between runs is safe: nothing outside this module can
    write to what is in it, and {!advance} builds new arrays every frame. Where
    a run starts, and what a test starts from. *)

val down : actions -> control -> bool
(** Whether [control] is held down right now. The state, not the edge; use
    {!pressed} where it is the moment of pressing that matters. *)

val pressed : actions -> control -> bool
(** Whether [control] went down this frame. The edge, not the state: true for
    exactly one frame however long the control is then held, which is what a
    game wants for anything that should happen once per press. *)
...
(** How long [control] has been held, in seconds.
    ...
    of zero arrive together. It keeps its final value for the one frame on which
    {!released} is true. That last part lets a game tell a tap from a deliberate
    hold at the moment the control comes up: a door that opens on a press and
    commits on a long hold reads [released], then asks this how long the hold
    lasted. *)
...
(** Where the cursor is, in the framebuffer's coordinates. Meaningful only while
    the game has asked for a free cursor; under mouse look the cursor is pinned
    and this says nothing. *)

val with_pointer : actions -> int * int -> actions
(** The same frame with the cursor said to be somewhere else. What is held, what
    has just been pressed and how long it has been held are the frame's own and
    come through untouched.

    {!Camlcast_core.Engine} is what this is for, and its one caller.
    ...
    the frame — the one fact about a frame of input that somebody outside this
    module knows better. The only function here that builds an [actions] out of
    another one without rolling a frame forward. *)
...
(** Roll a frame forward onto what is held at this instant. [down] answers for
    ...
    [tapped] covers the control that went down and came back up in the gap
    ...
    frame, so {!pressed} is true now, {!released} is true on the frame after,
    and the hold reads the [0.] a tap is worth. A control that is both held and
    tapped is just held; nothing counts it twice. Omit [tapped] and nothing was
    ...
    you can write down: that is how everything here is tested, and how a game
...
module Runtime : sig
  (** The loop's half of this module: what {!Camlcast_core.Engine} calls once a
      ...
      prose is a sentence, not a boundary. Two of them do more than nothing if a
      game does call. {!Runtime.mouse_delta} reads SDL's accumulated motion
      ...
      The submodule prevents neither, and does not pretend otherwise. It takes
      them out of the namespace a game completes against and out of
      {!Camlcast.Input}, which does not re-export this. Reaching one therefore
      means naming [Camlcast_core] and this module, which is the boundary
  ...
  (** How far the mouse moved since the last call. Relative mouse mode (enabled
      ...
      SDL accumulates the delta until somebody reads it, so this must be called
  ...
  (** The same frame again, with nothing having changed and no time having
      ...
      rather than growing. A control let go of while nobody was looking arrives
      as an ordinary {!released} on the first frame the window is back, which is
      the first frame a game could have done anything about it.
      ...
      the seconds have been counted, and no suppression downstream can un-count
      them. *)
  ...
  (** Drain the event queue into a [queue], reading into the caller's event
      record rather than allocating one per event.

      The draining is the part that has to happen: the queue must be pumped
      every frame even when nothing in it is of interest, or the window stops
      ...
      remembered for {!sample}: a control the player taps inside a single frame
      is up again by the time the keyboard is read, so reading state alone loses
      it entirely — no press, no release, nothing at all. Coming {e up} is not
      remembered; the device already reports a control that is up as up.
      Auto-repeat is not filtered out, because a repeat means the key is held,
      and the mark it leaves is one the state was going to make anyway.

      Still lost is the control released {e and} pressed again inside one frame.
      It is down at both ends of this frame and was down at both ends of the
      last, so there is no edge to detect without keeping the whole queue in
      ...
      frame per press, which is all that watching for the event would buy.
  ...
  (** A frame in which nothing whatever went past. What the first sample of a
      run starts from; there is no queue to drain before the run has begun. *)
  ...
  (** Read the keyboard and the mouse buttons as they are now, add what the
      ...
      The keyboard array SDL hands back is SDL's own and changes underneath the
      program, so {!advance} copies out of it rather than keeping it. The cursor
      comes out in window coordinates; {!Camlcast_core.Engine} scales it into
      the framebuffer's with {!with_pointer}, being the one that knows how the
      two compare.
