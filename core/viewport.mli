(** How the window, whatever size it currently is, maps onto the camera.
    ...
    the window height, so {!Config.fov} is read as the horizontal field of view
    at {!Config.reference_aspect} and the vertical angle it implies is what is
    actually held fixed — dragging the window wider shows more to the left and
    right instead of magnifying what was already there.
...
(** Where a point of the world lands on the screen: [(column, row)] in pixels,
    ...
    screen at all. [point] is where it stands on the floor plan and [z] how high
    it is, both in the frame the pose is expressed in.
...
(** Where a sprite lands on the screen, in pixels. The pose is the player
    ...
    be one answer to where that is. {!Sight.t} carries the pose and the distance
    it needs. *)
...
(** How fast the middle of the screen rises with distance, at a given pitch: the
    ...
    horizon. It takes a pitch and not a viewport because it does not depend on
    one — a window of any size or shape points its crosshair at the same place
    in the world. {!Sight} is what needs this. *)
