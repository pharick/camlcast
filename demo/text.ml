(** {b Text on the screen.} A bitmap font, laid out and clipped over a finished
    frame.

    {!Camlcast_core.Font} is a picture of every glyph on a fixed grid, plus four
    numbers describing that grid. A character's place in the atlas is arithmetic
    on its code point and its advance is the cell width, so no metrics file has
    to stay in step with the picture. The price is monospaced text, which at
    this size would look nearly monospaced anyway.

    Four things shown:

    - The {b panel}: {!Camlcast_core.Font.wrap} breaks the paragraph to a pixel
      width and {!Camlcast_core.Font.measure} sizes the box behind it, so the
      box fits the text rather than the text being trusted to fit the box.
    - The {b colours}: one white atlas, tinted three ways.
      {!Camlcast_core.Paint.sub} multiplies it by the requested colour, the way
      {!Camlcast_core.Color.level} scales a surface pattern's colour.
    - The {b overlong line}: not truncated. Every glyph goes through
      {!Camlcast_core.Paint}, which clips against the buffer, so the line is cut
      where the screen stops.
    - The {b box} in the last line: a character the atlas has no cell for. It
      takes its space rather than closing up, because a substitution that
      changed the width would turn one wrong character into a wrong line.

    Overlay coordinates are the framebuffer's, not the window's — see
    {!Camlcast_core.Renderer.internal_size}. Resizing the window keeps the
    text's place and size relative to the screen; it only gets blockier or
    crisper. *)
...
(** The typeface, read once. [\127] is the atlas's 96th cell, a hollow box;
    naming it as the fallback puts something visible in the place of a character
...
(** The page, as a function of the buffer it has to fit in. Every measurement
    comes from Font rather than a literal: the panel is measured from the text
    and the count in the corner places itself by its own size. *)
let page ~viewport:(width, height) =
  let font = Lazy.force font in
  let lh = font.Font.height in
  let pad = 6 in
  (* A page of wrapped text, in a panel measured from the text. *)
  ...
  P.
    [
      ...
      (* Clipped, not truncated: this line is longer than the screen and
         nothing here measures it against the screen. *)
      text ~font ~x:(2 * pad)
        ~y:(y + (4 * lh))
        ~color:ink
      ...
      (* The count, bottom right, placed by measure rather than by a literal. *)
