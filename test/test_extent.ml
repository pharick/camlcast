...
(* Over a range small enough that the multiplication is safe, the division has
   to agree with it exactly: for positive integers [height <= limit / width]
   is [width * height <= limit], floor and all. That equivalence makes the
   rewrite a rewrite, not a conservative approximation. It is checked because
   a [<] for the [<=], or the operands the other way up, would still refuse
   the enormous rectangles below while quietly refusing a legitimate one at
   the boundary. *)
...
(* The case where multiplying cannot be trusted, the reason this module
   exists. OCaml's arithmetic wraps, so the product of the two largest extents
   there are is not enormous: it is [1]. A check written the obvious way says
   yes to it, and what follows is two arrays of one element under a record
   reporting a size of [max_int]. *)
...
(* The limit is an argument because it is not one number. Texture and Image
   pass [Sys.max_array_length]; Framebuffer passes
   [Sys.max_floatarray_length], the tighter bound its depth buffer has to
   satisfy. The two are equal at 64 bits and are not at 32, so on this machine
   no fixture can tell them apart. What can be pinned is that the argument
   decides, rather than a number written inside. *)
...
(* The size that made this worth wiring into {!Image.load} and {!Texture.load}
   ...
   because a bad file is an expected condition; reaching the allocation
   instead would put an [Invalid_argument] through a signature that says a bad
   file comes back as a value. Nothing here can be reached on the 64-bit build
   these tests run on, which is why the limit is passed rather than read from
   [Sys]: the arithmetic is the part that has to be right on a machine this is
   not.

   The pair either side of the limit matters. One texel less and the same
   picture is fine, so this pins a boundary rather than the general shape of a
   refusal. *)
