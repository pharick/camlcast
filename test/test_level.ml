...
let a_portal_knows_its_twin () =
  List.iter
    (fun (room, _, p) ->
      ...
      Alcotest.check vec "endpoints meet, reversed" back.World.threshold.Room.b
...
let the_default_world_is_playable () =
  let spawn = World.spawn Level.default in
  Alcotest.(check bool)
...
let the_default_world_is_connected () =
  ...
  in
  visit (World.spawn Level.default).room;
  Alcotest.(check bool)
    "every room is reachable from the spawn" true
    (Array.for_all Fun.id seen)

let the_default_world_is_varied () =
  let all = rooms Level.default in
  Alcotest.(check bool)
    "some room is open to the sky" true
    (List.exists (fun (r : Room.t) -> ceiling_is_open r) all);
  Alcotest.(check bool)
