(* Step 13 of doc/making-a-game.mld — "Words". The guide quotes only what
   each step adds; this file is the whole game as of this step.

   New here: a font over an atlas the game supplies, a line of instructions
   wrapped to the buffer's width, and Hook.use_memo remembering the wrap
   until the width changes. *)
