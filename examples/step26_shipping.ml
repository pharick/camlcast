...
type art = { font : Font.t; tiles : Material.t; poster : Image.t }

let game =
  Element.declare ~name:"game" @@ fun (art : art) ->
  let font = art.font in
       ...
       P.(
         world
           ...
           [
             ...
             room ~height ~material:stone ~floor:(floor ground)
               ~ceiling:(roof stone)
               ~outline:(corners [ c_sw; c_se; c_ne; c_nw ])
               [
                 cut west ~along:(c_sw, c_nw);
                 (* The gate: a brick lintel over an iron grille, both arguments to
                    the cut rather than walls of their own. Both are this room's own
                    answer — the courtyard gives its side a leaf too, and need not
                    give it the same lintel. *)
                 cut gate_out
                   ~leaf:
                     (Door.make
             ...
             room ~height ~material:stone ~floor:(floor ground)
               ...
               [
                 (* The same leaf, said again: one leaf hangs in one opening and
                    both rooms draw their side of it, so both say so. *)
                 cut gate_in
                   ~leaf:
                     (Door.make
                        ~state:(if gate_open then Open else Closed)
...
(* Everything read from disk, gathered in one place and one failure. Asset
   looks beside the executable, one directory above it, and wherever
   CAMLCAST_ASSETS points — which is why `dune build` first, or the
   environment variable, is all the setup a run needs, and why a bundled
   copy needs only its pictures put beside its binary. *)
let load_art () =
  let ( let* ) = Result.bind in
  let* atlas = Image.of_asset "assets/font.png" in
  let* tiles = Texture.of_asset "assets/tiles.png" in
  let* poster = Image.of_asset "assets/poster.png" in
  Ok
    {
      font = Font.make ~fallback:'\127' ~atlas ~width:6 ~height:10 ~first:32 ();
      tiles = Material.make ~pattern:tiles;
      poster;
    }

(* A title screen is a world like any other, with two of the parts this
   game has not needed yet: cursor, which frees the mouse for a screen drawn
   over the world, and a camera swaying where a player would stand. Enter
   describes finish, and finish is how a run on a shared window hands the
   window back. *)
let title_screen =
  Element.declare ~name:"title" @@ fun (font : Font.t) ->
  let descending, set_descending = Hook.use_state false in
  Events.use_pressed (Input.Key Key.return) (fun () -> set_descending true);
  let sway = Hook.use_ref 0. in
  Events.use_frame (fun ~dt -> sway := !sway +. dt);
  let w, h = Events.use_viewport () in
  let centered line y =
    let tw, _ = Font.measure font line in
    P.text ~font ~x:((w - tw) / 2) ~y line
  in
  P.(
    world ~atmosphere:(air ~fire:0.4)
      [
        ...
        hud
          [
            centered "THE UNDERCROFT" (h / 3);
            centered "press Enter to descend" ((h / 3) + 16);
          ];
      ])

(* The title has no way out of its own: Enter goes down, and the close
   button is still the close button. *)
let title_controls = Controls.make ~bindings:(Binding.make ()) ()
...
let prove art =
  ...
  let frame dt =
    Element.provide Events.context { Events.still with dt } [ game art ]
  in
...
let () =
  ...
  | Ok art -> (
      ...
      (* A window and a run are two lifetimes. The window is opened once;
         title and game take turns on it, and coming back to the title is
         the picture changing rather than the window blinking. *)
      let outcome =
        Run.with_window ~title:"The Undercroft" (fun window ->
            let rec descend () =
              match
                Run.on window ~controls:title_controls (title_screen art.font)
              with
              | Ok Returned -> (
                  match Run.on window ~controls (game art) with
                  | Ok Returned ->
