(* Step 8 of doc/making-a-game.mld — "State". The guide quotes only what
   each step adds; this file is the whole game as of this step.

   New here: the brazier holds whether it is lit, and Space strikes it. Two
   rules come with hooks: call them unconditionally in the same order, and a
   setter shows up next frame. *)
...
(* State: what this brazier is holding, and a way to replace it. The setter
