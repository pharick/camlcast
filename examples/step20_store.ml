(* Step 20 of doc/making-a-game.mld — "The store". The guide quotes only
   what each step adds; this file is the whole game as of this step.

   New here: Store replaces the hand-rolled box, listeners and notify of
   step 19 with create, dispatch and use_selector — the same seam, packaged,
   with a reducer as the one door state changes go through. *)
