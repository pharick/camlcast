(* Step 6 of doc/making-a-game.mld — "A second room". The guide quotes only
   what each step adds; this file is the whole game as of this step.

   New here: a corridor east of the vault. Three of the vault's sides run as
   an open boundary, the fourth is a doorway, and a link says the corridor's
   own doorway is the same one. The corridor's floor is carried through the
   opening rather than written again. *)
