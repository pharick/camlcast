(* Step 16 of doc/making-a-game.mld — "Held controls". The guide quotes only
   what each step adds; this file is the whole game as of this step.

   New here: the two remaining ways of reading input. Events.use_down is the
   hold — Shift cups the brazier's flame and stops it burning down — and
   Events.use_actions is everything else, here how long E has been held at
   the winch that raises the gate. *)
