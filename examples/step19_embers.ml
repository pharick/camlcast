(* Step 19 of doc/making-a-game.mld — "Shared state, by hand". The guide
   quotes only what each step adds; this file is the whole game as of this
   step.

   New here: embers to gather, and a count of them that belongs to nobody in
   particular — a box, a list of listeners, and Hook.use_invalidate as the
   seam through which the outside world asks a component's root for a frame.
   The plumbing is deliberately bare; the next step replaces it whole. *)
...
(* A score the whole game shares, by hand: a box, whoever is listening, and
   a promise to tell them. Fifteen lines that Store will replace whole. *)
let embers_taken = ref 0
let listeners : (unit -> unit) list ref = ref []

let subscribe listener =
  listeners := listener :: !listeners;
  fun () -> listeners := List.filter (fun l -> l != listener) !listeners

let take_ember () =
  incr embers_taken;
  List.iter (fun listener -> listener ()) !listeners
...
(* The tally reads the shared box. It subscribes once, on mount; the
   listener asks this component's root for a frame through use_invalidate;
   and the cleanup lets go when the tally leaves the tree. Store is exactly
   this, packaged. *)
let tally =
  Element.declare ~name:"tally" @@ fun (font : Font.t) ->
  let invalidate = Hook.use_invalidate () in
  Hook.use_effect ~deps:() (fun () -> Some (subscribe invalidate));
  let w, _ = Events.use_viewport () in
  let line = Printf.sprintf "embers %d" !embers_taken in
...
let () =
  ...
  | Ok font -> (
      match
        Camlcast_edit.Session.play ~title:"The Undercroft" studio (game font)
      with
      | Ok _ending -> ()
      | Error (`Msg reason) ->
          prerr_endline reason;
