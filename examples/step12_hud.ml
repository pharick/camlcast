(* Step 12 of doc/making-a-game.mld — "Drawing over the world". The guide
   quotes only what each step adds; this file is the whole game as of this
   step.

   New here: a HUD — a panel, a fuel meter that eases toward the truth, a
   ring round whatever the crosshair rests on, and the crosshair itself. All
   of it is placed in the buffer's pixels, which are not the window's, so the
   viewport is asked for rather than assumed. *)
