(* Step 21 of doc/making-a-game.mld — "Context". The guide quotes only what
   each step adds; this file is the whole game as of this step.

   New here: a context carrying how alive the flames are — provided once,
   above the world, and read by the torches and the wisp with no props
   between; cupping the brazier now dims every flame at once. And a keyed
   fragment, for the pair of motes that is one thing to the description. *)
...
let game_store =
  Store.create ~initial:{ embers = 0 } ~reducer:(fun state Ember_taken ->
