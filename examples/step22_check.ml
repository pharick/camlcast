(* Step 22 of doc/making-a-game.mld — "Prove it without a window". The
   guide quotes only what each step adds; this file is the whole game as of
   this step.

   New here: the level is read by Check, rendered by Mount with no window
   anywhere, driven a fake frame by providing Events.context, and its store
   is exercised on a scratch copy — all before SDL is asked for anything.
   `dune exec examples/step22_check.exe -- --check` runs only the proof. *)
