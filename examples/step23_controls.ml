(* Step 23 of doc/making-a-game.mld — "Controls, and the map". The guide
   quotes only what each step adds; this file is the whole game as of this
   step. README.md quotes the controls value below.

   New here: the one record the loop acts on by itself — walking gains a
   second pair of keys, the left mouse button works what E works, and the
   debug map answers M as well as F3. *)
