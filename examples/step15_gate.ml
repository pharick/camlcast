(* Step 15 of doc/making-a-game.mld — "A door in the doorway". The guide
   quotes only what each step adds; this file is the whole game as of this
   step.

   New here: a courtyard past the corridor, barred by an iron grille — a
   Door, shut, in a threshold whose jambs the game builds itself so a brick
   lintel can hang over it. The grille's material is generate_masked: colour
   on the bars, nothing between them, so the way out is visible long before
   it opens. *)
      ...
      [
        ...
        room ~height ~material:stone ~floor:(floor ground) ~ceiling:(roof stone)
          ...
          [
            ...
            (* The gate: a brick lintel over a shut iron grille. Both are
               arguments to the cut rather than walls of their own, and both
               are this room's own answer — the courtyard gives its side a
               leaf too, and need not give it the same lintel. *)
            cut gate_out
              ~leaf:(Door.make ~state:Closed grille)
