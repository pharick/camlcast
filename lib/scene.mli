(** What a frame of description comes to.
    ...
    One frame yields:
    - a world;
    - where the eye is, if the description said;
    - whether the run is over;
    - whether the pointer is loose;
    - what a HUD tree draws over the top;
    - every handler the description hung on something the crosshair can land on.

    A record rather than a bare {!Camlcast_core.World.t} because a frame turned
    out to be all of that. It began as the world alone, and each later field
    arrived without any caller that matched on the record having to be
    rewritten. *)
...
type t = {
  ...
  camera : Player.t option;
      (** where the description said the eye is, if it said.
          ...
          that has taken the camera over — a cutscene, a lift, a death. This is
  ...
  hud : Prim.t list;
      (** what to draw over the finished world, in the order it was written.
          ...
          draw. {!Overlay.draw} turns them into pixels. *)
}
