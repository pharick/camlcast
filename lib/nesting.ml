...
(* The parent passed down is always the node's own primitive. A previous bug in
   Host checked a hud's grandchildren as though the hud were their parent, and
   so allowed a bar inside a rectangle. Nothing here may substitute one parent
   for another. *)
