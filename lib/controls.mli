(** What a run does with the player's controls by itself.
    ...
    to arrive by three different mechanisms:
    - walking and looking, through a {!Camlcast_core.Binding.t};
    - working whatever the crosshair is on, through a single control;
    - the map over the world, through a boolean with a key baked in behind it.

    Rebinding any one of them meant knowing which of the three mechanisms it was
    on, and one of them could not be rebound at all.

    This record is the one mechanism that replaced them. Every field is a part
    of what {!Run} acts on before any component has run. Each of the last two is
    a list of {!Camlcast_core.Input.control}s, so a key and a mouse button can
    do the same job; an empty list is how a game turns one off.
...
type t = {
  ...
  map : Input.control list;
      (** any of these turns {!Debug_map} on and off over the world.
          ...
          Empty is a game that has stopped wanting a map over its world. That
          costs the game nothing: the walk over every wall of every room that
          feeds the map only happens while the map is up. *)
}
...
(** {!default}, with the given parts replaced.
    ...
    Replacement and not merging, exactly as in {!Camlcast_core.Binding.make}: a
    given [~use] is the whole of what works a door, not one more control that
    also does. *)

val default : t
(** [E] to work whatever the crosshair is on, [F3] for the map, and
    ...
    played on a window of its own has nothing else to end it, so this is the
    layer that assumes it. A game with a pause screen of its own takes the key
    back with [~bindings]. *)
